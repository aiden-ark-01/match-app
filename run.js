...
(async () => {
  try {
    ...
    //开始构建镜像
    const buildImage = async (image,num=10) => {
      try {
        console.log(`开始构建${image}镜像`);
        const { stdout } = await exec(`docker build -t ${image} ./${image}`);
        log(`docker build -t ${image} ./${image}`, stdout);
      } catch (error) {
        if(num>0){
        console.log(`镜像${image}构建失败,继续执行,剩余尝试次数:`,num);
        await sleep(2000);
        await buildImage(image,num-1)
        }else{
          throw error
        }
      }
    ...
    for (let i = 0; i < images.length; i++) {
      await buildImage(images[i]);
    }
    console.log('镜像构建成功!\n');

    //开始创建集群
    const createStack = async (stack,num=10) => {
      try {
        console.log(`开始创建${stack}集群`);
        const { stdout } = await exec(`docker stack deploy -c docker-compose.yml ${stack}`);
        log(`docker stack deploy -c docker-compose.yml ${stack}`, stdout);
      } catch (error) {
        if(num>0){
        console.log('执行失败,继续执行,剩余尝试次数:',num);
        await sleep(2000);
        await createStack(stack,num-1)
        }else{
          throw error
        }
      }
